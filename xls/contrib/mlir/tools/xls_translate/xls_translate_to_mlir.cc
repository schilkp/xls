...
absl::StatusOr<Operation*> translateLiteral(const ::xls::Value& b,
  ...
  switch (b.kind()) {
    case ::xls::ValueKind::kBits:
      return translateBitsLiteral(b.bits(), builder, ctx, func_ctx);
    ...
    case ::xls::ValueKind::kArray: {
      ...
      for (auto& xls_member : b.elements()) {
        auto member = translateLiteral(xls_member, builder, ctx, func_ctx);
        if (!member.ok()) {
...
absl::StatusOr<Operation*> translateOp(::xls::ArithOp& node, OpBuilder& builder,
  ...
  switch (node.op()) {
    case ::xls::Op::kUMul:
      return builder.create<xls::UmulOp>(builder.getUnknownLoc(), result_type,
                                         *opr_lhs, *opr_rhs);
    case ::xls::Op::kSMul:
      return builder.create<xls::SmulOp>(builder.getUnknownLoc(), result_type,
                                         *opr_lhs, *opr_rhs);
    default:
      return absl::InternalError(absl::StrCat(
...
absl::StatusOr<Operation*> translateOp(::xls::PartialProductOp& node,
                                       ...
                                       FunctionContext& func_ctx) {
  auto opr_lhs = func_ctx.get_ssa(
      node.operands()[::xls::PartialProductOp::kLhsOperand]->id());
  if (!opr_lhs.ok()) {
    return opr_lhs.status();
  }

  auto opr_rhs = func_ctx.get_ssa(
  ...
  switch (node.op()) {
    case ::xls::Op::kSMulp:
      op = builder.create<xls::SmulpOp>(builder.getUnknownLoc(), result_type,
                                        *opr_lhs, *opr_rhs);
      break;
    case ::xls::Op::kUMulp:
      op = builder.create<xls::UmulpOp>(builder.getUnknownLoc(), result_type,
                                        *opr_lhs, *opr_rhs);
      break;
    default:
      return absl::InternalError(
  ...
  return builder.create<xls::TupleOp>(builder.getUnknownLoc(), result_tuple);
}
...
absl::StatusOr<Operation*> translateOp(::xls::TupleIndex& node,
  ...
  return builder.create<xls::TupleIndexOp>(builder.getUnknownLoc(), result_type,
                                           *operand, index);
}
...
absl::StatusOr<Operation*> translateOp(::xls::DynamicBitSlice& node,
  ...
  return builder.create<xls::DynamicBitSliceOp>(
      builder.getUnknownLoc(), result_type, *arg, *start,
...
absl::StatusOr<Operation*> translateOp(::xls::Concat& node, OpBuilder& builder,
  ...
  return builder.create<xls::ConcatOp>(builder.getUnknownLoc(), result_type,
                                       operands);
}
...
absl::StatusOr<Operation*> translateOp(::xls::Literal& node, OpBuilder& builder,
  ...
  switch (node.GetType()->kind()) {
    case ::xls::TypeKind::kBits:
      return translateBitsLiteral(node.value().bits(), builder, ctx, func_ctx);
    default: {
      auto result_type = translateType(node.GetType(), builder, ctx);

      // Create literal with region:
      auto literal_op =
          builder.create<xls::LiteralOp>(builder.getUnknownLoc(), result_type);
      builder.setInsertionPointToStart(&literal_op.getRegion().emplaceBlock());

      // Construct value using ops:
      auto final_op = translateLiteral(node.value(), builder, ctx, func_ctx);
      if (!final_op.ok()) {
...
absl::StatusOr<Operation*> translateOp(::xls::NaryOp& node, OpBuilder& builder,
  ...
  switch (node.op()) {
    ...
    case ::xls::Op::kNand:
      return builder.create<xls::NandOp>(builder.getUnknownLoc(), operands);
    case ::xls::Op::kOr:
      return builder.create<xls::OrOp>(builder.getUnknownLoc(), operands);
    case ::xls::Op::kNor:
      return builder.create<xls::NorOp>(builder.getUnknownLoc(), operands);
    case ::xls::Op::kXor:
...
absl::StatusOr<Operation*> translateOp(::xls::BitwiseReductionOp& node,
  ...
  switch (node.op()) {
    case ::xls::Op::kAndReduce:
      return builder.create<xls::AndReductionOp>(builder.getUnknownLoc(),
                                                 result_type, *operand);
    case ::xls::Op::kOrReduce:
      return builder.create<xls::OrReductionOp>(builder.getUnknownLoc(),
                                                result_type, *operand);
    case ::xls::Op::kXorReduce:
      return builder.create<xls::XorReductionOp>(builder.getUnknownLoc(),
                                                 result_type, *operand);
    default:
      return absl::InternalError(
...
absl::StatusOr<Operation*> translateOp(::xls::Gate& node, OpBuilder& builder,
                                       MLIRContext* ctx,
                                       FunctionContext& func_ctx) {
  auto data = func_ctx.get_ssa(node.data()->id());
  if (!data.ok()) {
    return data.status();
  }

  auto condition = func_ctx.get_ssa(node.condition()->id());
  if (!condition.ok()) {
    return condition.status();
  }

  return builder.create<xls::GateOp>(builder.getUnknownLoc(), *condition,
                                     *data);
}
...
absl::StatusOr<Operation*> translateOp(::xls::CountedFor& node,
                                       ...
                                       FunctionContext& func_ctx,
                                       PackageContext& pkg_ctx) {
  auto initial_value = func_ctx.get_ssa(node.initial_value()->id());
  if (!initial_value.ok()) {
  ...
  std::vector<Value> invar_args_vec{};
  for (auto xls_operand : node.invariant_args()) {
    auto invar_arg = func_ctx.get_ssa(xls_operand->id());
    if (!invar_arg.ok()) {
  ...
  auto body = pkg_ctx.get_fn(node.body()->name());
  if (!body.ok()) {
    return body.status();
  }
  ...
  return builder.create<xls::CountedForOp>(builder.getUnknownLoc(), result_type,
                                           *initial_value, invar_args,
                                           trip_count, *body, stride);
}
  ...
  for (auto n : xls_func.nodes()) {
    ...
    if (n->Is<::xls::Param>()) {
      ...
      if (xls_func.return_value() == n) {
        auto maybe_return_value = func_ctx.get_ssa(n->As<::xls::Param>()->id());
