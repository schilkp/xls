...
class TranslationState {
 public:
  ...
  absl::Status setFn(std::string name, FlatSymbolRefAttr fn) {
  ...
  absl::StatusOr<FlatSymbolRefAttr> getFn(std::string name) {
  ...
  absl::Status setChn(std::string name, FlatSymbolRefAttr chn) {
  ...
  absl::StatusOr<SymbolRefAttr> getChn(std::string name) {
  ...
  void newFunctionBaseContext(bool is_proc) {
    this->ssa_map_ = {};
    this->is_proc_ = is_proc;
    this->state_elem_map = {};
  }

  void setStateElem(std::string name, Value elem) {
    assert(is_proc_ && "Only proc functions contain state elements");
    state_elem_map[name] = elem;
  }

  absl::StatusOr<Value> getStateElem(std::string id) {
    assert(is_proc_ && "Only proc functions contain state elements");
 ...
 private:
  ...
  std::unordered_map<::xls::Fileno, StringAttr> fileno_map_;
...
absl::StatusOr<Operation*> translateOp(::xls::Invoke& node, OpBuilder& builder,
                                       MLIRContext* ctx,
                                       TranslationState& state) {
  auto fn = state.getFn(node.to_apply()->name());
  if (!fn.ok()) {
    return fn.status();
  }
...
absl::StatusOr<Operation*> translateOp(::xls::ChannelNode& node,
                                       OpBuilder& builder, MLIRContext* ctx,
                                       TranslationState& state) {
  auto chn = state.getChn(node.channel_name());
  if (!chn.ok()) {
    return chn.status();
  }
...
absl::StatusOr<Operation*> translateOp(::xls::CountedFor& node,
  ...
  auto body = state.getFn(node.body()->name());
  if (!body.ok()) {
    return body.status();
  }
...
absl::StatusOr<Operation*> translateFunction(::xls::Function& xls_func,
  ...
  if (auto err =
          state.setFn(xls_func.name(), SymbolRefAttr::get(func.getNameAttr()));
      !err.ok()) {
    return err;
  }
...
absl::StatusOr<Operation*> translateProc(::xls::Proc& xls_proc,
  ...
  for (int64_t i = 0; i < xls_proc.StateElements().size(); i++) {
    ...
    state.setStateElem(xls_elem->name(), mlir_elem);
  }
  ...
  for (auto n : xls_proc.nodes()) {
    ...
    if (auto state_read = dynamic_cast<::xls::StateRead*>(n)) {
      auto state_value =
          state.getStateElem(state_read->state_element()->name());
      if (!state_value.ok()) {
  ...
  if (!is_legacy_proc) {
    for (auto state_elem : xls_proc.StateElements()) {
      ...
      if (elem_next_value_ops.size() == 1 &&
        ...
        // NextValueOp needed - directly feed ProcYieldOp from value:
  ...
  builder.setInsertionPointToEnd(body);
  builder.create<ProcYieldOp>(builder.getUnknownLoc(),
...
absl::Status translateChannel(::xls::Channel& xls_chn, OpBuilder& builder,
  ...
  return state.setChn(std::string(xls_chn.name()),
