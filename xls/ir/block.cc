...
absl::Status Block::ReplaceInstantiationWith(
  ...
  for (InstantiationInput* inp : inps) {
    ...
    auto new_name =
        renaming_rule == port_renaming.end() ? inp->port_name() : renaming_rule->second;
    XLS_RETURN_IF_ERROR(
        inp->ReplaceUsesWithNew<InstantiationInput>(inp->data(), new_inst, new_name)
            .status());
  ...
  for (InstantiationOutput* out : outs) {
    ...
    auto new_name =
        renaming_rule == port_renaming.end() ? out->port_name() : renaming_rule->second;
    XLS_RETURN_IF_ERROR(
